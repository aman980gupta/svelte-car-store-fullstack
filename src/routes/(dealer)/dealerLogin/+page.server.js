...
export const actions = {
    login: async (event) => {
        ...
        const dealerFromDatabase = await connection.db("test").collection("dealers").findOne({ dealer_email });
        if (!dealerFromDatabase) return { success: false, error: true, message: "no dealer fount",}
        const passwordMatched = await bcrypt.compareSync(dealer_password, dealer.password)
        if (!passwordMatched) return { success: false, error: true, message: "password is wrong", dealerFromDatabase }
